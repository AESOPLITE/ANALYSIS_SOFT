...
public:
    Histogram2D (int nXbins, double Xbin0, double XbinWidth, int nYbins, double Ybin0, double YbinWidth, string title, string xlabel, string ylabel, string zlabel);
    void entry(double x, double y);
    void entry(int i, int j);
    void plot(FILE* oFile);
    bool add(Histogram2D *otherHist, string Title);
    Histogram columnSlice(int i);
    Histogram yProj();
    string Title() {
        return T;
    }
...
// Class to make "profile plots" in Gnuplot, i.e. a histogram that displays the mean and error on the mean in each bin.
// To do this, when processing data the sum and sum-squared have to be accumulated in addition to the number of counts, for each bin.
class ProfilePlot {
    int N;
    double BW, B0; 
    string T, XL, YL;
...
public:
    ProfilePlot2D(int nxBins, double xbin0, double xbinWidth, int nyBins, double ybin0, double ybinWidth, string title, string xlabel, string ylabel, string zlabel);
...
public:
    ProfilePlot2Dpeak(int nxBins, double xbin0, double xbinWidth, int nyBins, double ybin0, double ybinWidth, int nzBins, double zbin0, double zbinWidth, 

// Simple classes to make histograms and profile plots, using Gnuplot to do the plotting
// R.P. Johnson   5/22/2016

#include "Histogram.h"

Histogram::Histogram(int nBins, double bin0, double binWidth, std::string title, std::string xlabel, std::string ylabel) {
    N= nBins;
    BW= binWidth;
    B0= bin0;
    T= title;
...
void Histogram::plot(FILE* oFile, bool stats, std::string choice, std::string txt) {  // Create a Gnuplot file to display the histogram
    X.resize(N);
    Y.resize(N);
    Ex.resize(N);
    Ey.resize(N);
...
void Histogram::print(std::string fn) {  // Print the histogram contents to an ASCII file
...
double Histogram::mean(float min, float max) {  // Return the mean within a range
    ...
    double avg = 0.;
    if (tCnts > 0) avg = W/((double)tCnts);
    return avg;
}

double Histogram::mean() { // Return the mean of the full distribution
    double avg = 0.;
    if (nEntry > 0) avg = sumX/float(nEntry);
    return avg;
}

Histogram2D::Histogram2D(int nXbins, double Xbin0, double XbinWidth, int nYbins, double Ybin0, double YbinWidth, string title, string xlabel, string ylabel, string zlabel) {
    NX= nXbins;
    NY= nYbins;
    BXW= XbinWidth;
    BYW= YbinWidth;
    ...
    ZL= zlabel;
    nEntry = 0;
    counts.resize(nXbins, vector<int>(nYbins, 0));
};
...
// Return a column of the 2D histogram as a 1D histogram:
Histogram Histogram2D::columnSlice(int i) {
    string Title;
...
void Histogram2D::plot(FILE* oFile) {
    ...
    fprintf(oFile,"set nokey\n");
    fprintf(oFile,"#*** Hidden 3d makes it slow to plot and manipulate the graph; not worth it usually.\n");
...
ProfilePlot2D::ProfilePlot2D(int nxBins, double xbin0, double xbinWidth, int nyBins, double ybin0, double ybinWidth, string title, string xlabel, string ylabel, string zlabel) {
    Nx = nxBins;
    Ny = nyBins;
    xB0 = xbin0;
    yB0 = ybin0;
...
void ProfilePlot2D::plot(FILE* oFile) {
    ...
    for (int i=0; i<Nx; ++i) {
        for (int j=0; j<Ny; ++j) {
            ...
            double Z;
            if (counts[i][j]>0) Z = sumZ[i][j]/(double)counts[i][j]; else Z = 0.;
            //double Ez;
            //if (counts[i][j] > 1) Ez= sqrt((sumZ2[i][j]/(double)counts[i][j] - Z*Z)/((double)(counts[i][j]-1))); else Ez = 0.;
            if (counts[i][j] > 4) fprintf(oFile,"%8.3e %8.3e %8.3e\n",X,Y,Z);
        }
        fprintf(oFile," \n");
    }
    fprintf(oFile,"EOD\n");
...
ProfilePlot::ProfilePlot(int nBins, double bin0, double binWidth, std::string title, std::string xlabel, std::string ylabel) { 
    N= nBins;
    BW= binWidth;
    B0= bin0;
    T= title;
...
void ProfilePlot::print(std::string fn) {
    ...
    if (oFile != NULL) {
        ...
        for (int i=0; i<N; ++i) {
            ...
            XX= B0 + (((double)i)+0.5)*BW;
            C= counts[i];
            if (counts[i] > 0) YY= sumY[i]/(double)counts[i]; else YY = 0.;
            if (counts[i] > 1) EEy= sqrt((sumY2[i]/(double)counts[i] - YY*YY)/((double)(counts[i]-1))); else EEy = 0.;
...
void ProfilePlot::plot(FILE* oFile) {
    ...
    for (int i=0; i<N; ++i) {
        if (counts[i] > 0) {
            ...
            Ex[i]= BW/2.;
            if (counts[i] > 1) Ey[i]= sqrt((sumY2[i]/(double)counts[i] - Y[i]*Y[i])/((double)(counts[i]-1)));
            else Ey[i] = 0.;
        }
        else {
            X[i]= B0 + (((double)i)+0.5)*BW;
...
// This class used a histogram in each bin of the 2D space to find the peak of the distribution in the bin, instead of the average value used in the normal profile plot
// Consequently it uses a lot of memory but works well for the radiograph
ProfilePlot2Dpeak::ProfilePlot2Dpeak(int nxBins, double xbin0, double xbinWidth, int nyBins, double ybin0, double ybinWidth, int nzBins, double zbin0, double zbinWidth, 
...
void ProfilePlot2Dpeak::print(FILE* oFile) {
    ...
    for (int i=0; i<Nx; ++i) {
        ...
        for (int j=0; j<Ny; ++j) {
            ...
            fprintf(oFile,"%8.3e ",Z);
/*			if (i>=40 && i<=59 && j==40) {
				string fn = "hist_" + to_string((long long int)i) + "_" + to_string((long long int)j) + ".gp";
				FILE* dFile= fopen(fn.c_str(),"w");
				counts[i][j]->plot(dFile);
				float mode = counts[i][j]->mode();
				float mean = counts[i][j]->mean();
				cout << "ProfilePlot2Dpeak::print: hist " << i << "," << j <<"100,80, xLow=" << xLow << " xHigh=" << xHigh << " Z=" << Z << " mode=" << mode << " mean= " << mean << endl;
			}  */
        }
        fprintf(oFile," \n");
    }

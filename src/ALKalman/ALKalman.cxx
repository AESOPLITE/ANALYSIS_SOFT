...
void ALKalman::InitialBackwardFit(TObjArray &kalhits, THelicalTrack &Hel_1st,TKalMatrix &C_1st) 
{
	...
	cout << "CALLING FUNCTION InitialBackwardFit ... " << endl;
   ...
   //loop over the entries in kalhit buffer
	  ...
	  cout << "Initial 3 points helix param:	rho=" << helstart.GetDrho() << " Phi0=" << helstart.GetPhi0() << "  , kappa=" << helstart.GetKappa() << "  , dz=" << helstart.GetDz() << "  TanL=" << helstart.GetTanLambda() << endl;


     static TKalMatrix C(kSdim,kSdim);
      for (Int_t i=0; i<kSdim; i++) {
         C(i,i) = 0.0005;   // dummy error matrix
      }
  ...
  theLastState->GetCovMat().DebugPrint();
  kaltrack.GetCurSite().GetCurState().DebugPrint("Final State Vector");
...
// TObjArray rawhits; 	
	...
	Bool_t magfield = bfield->SetMagField();
	 bool bApply2Iter = true;									//if initialize with first fit
 ...
 for (int i=0;i<6;i++) {
   ...
   //loop over the entries in kalhit buffer
    ...
    InitialBackwardFit(kalhits, helstart, C_start);
      ...
      svd(4,0) = helstart.GetTanLambda();
      if (kSdim == 6) svd(5,0) = 0.;
	  cout << "Initialized helix from first iteration of Kalman Filter:	rho=" << helstart.GetDrho() << " Phi0=" << helstart.GetPhi0() << "  , kappa=" << helstart.GetKappa() << "  , dz=" << helstart.GetDz() << "  TanL=" << helstart.GetTanLambda() << endl;

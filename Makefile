...
# ----------------------------------------------------------------------
# Makefile generated from "Imake.tmpl" and </tmp/IIf.aUS3ty>
...
# operating system:  Linux 4.4.0-165-generic x86_64 [ELF] (4.4.0)
# libc:	(6.23.0)
# binutils:	(226)
...
# #---------------------------------------------------
# # Machine dependant configuration to build application.
# #---------------------------------------------------
# #
# #  Required environment variable.
# #     ROOTSYS : ROOT top directory.
# #
# #(History)
# #  29-May-1999 Add EXTRACXXOPT in CXXFLAGS
# #              Define .cxx compile rule here.
# #  4-Feb-2000 soname option in SOFLAGS for Linux is removed.
# #
# # $Id: makejsf.tmpl,v 1.10 2010-04-06 06:45:07 fujiik Exp $
# #

ROOT_VERSION = 6
ROOT_MINORVERSION = 8
ROOT_PATCHLEVEL= 0
ROOT_FULLVERSION= 60800
G77EGCS_VERSION = 5
G77EGCS_MINORVERSION = 4
G77EGCS_PATCHLEVEL = 0
UNAME_MACHINE = x86_64
UNAME_SYSNAME = Linux
G77EGCS_LIBNAME = gfortran
...
CXX           = g++ -O2 -g
CC            = $(CXX)
CXXFLAGS      = -Wall -fPIC 		$(EXTRACXXOPT) 		$(shell root-config --cflags)

SOFLAGS       = -shared -rdynamic
LDFLAGS       = $(ANALLIB) $(GLIBS) -rdynamic
LIBS          = $(ROOTLIBS) -lm -ldl
GLIBS         = $(ROOTLIBS) $(ROOTGLIBS) -L/usr/X11R6/lib 		-lXpm -lX11 $(LIBF2C) -lm -ldl
...
# # -----------------------------------------------------
# # For Machten gcc (This is not tested yet.)
# # -----------------------------------------------------
...
SRCS          =  $(PROGRAMNAME).$(SrcSuf)                 $(SRCDIR)/ALSim/MakeRawBPDEvent.$(SrcSuf) 		$(SRCDIR)/ALSim/MakeEventData.$(SrcSuf)    	     	$(SRCDIR)/ALSim/MakeRawEventMC.$(SrcSuf)  	        $(SRCDIR)/ALSim/MakeRawEventAtmoMC.$(SrcSuf) 		$(SRCDIR)/ALKalman/ALKalman.$(SrcSuf) 	        $(SRCDIR)/ALPatternRecognition/ALPatternRecognition.$(SrcSuf) 	        $(SRCDIR)/RKFitter/RKfitter.$(SrcSuf)                 $(SRCDIR)/RKFitter/TkrData.$(SrcSuf)                 $(SRCDIR)/RKFitter/FieldMap.$(SrcSuf)                 $(SRCDIR)/RKFitter/RungeKutta4.$(SrcSuf)

OBJS	      =	$(subst .$(SrcSuf),.$(ObjSuf),$(SRCS)) 		$(PROGRAMNAME)Dict.$(ObjSuf)
...
$(PROGRAM): $(OBJS) dir
	$(LD) -o $(PROGRAM) $(OBJS) 	      -L$(LIBINSTALLDIR) -lS4ALSim -lS4Geom -lS4KalTrack -lS4Kalman  -lS4Utils -lS4ALPatternReco -lS4ALKalman 	      $(LDFLAGS)

cleandir::
	@rm -f $(OBJS) prod/core

depend:: $(SRCS) $(HDRS)
	for i in $(SRCS); do 	rmkdepend -a -- $(CXXFLAGS) $(INCPATH) $(DEPENDFILES) -- $$i; done
...
$(DICTNAME).$(SrcSuf): $(HDRS) LinkDef.h
	...
	rootcint -f $(DICTNAME).$(SrcSuf) 	         -c -I$(INCINSTALLDIR) $(HDRS) LinkDef.h
